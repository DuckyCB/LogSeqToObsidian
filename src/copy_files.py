...
from markdown import is_markdown_file, is_empty_markdown_file
from text_utils import fix_escapes, unencode_filenames_for_links
    ...
    for root, dirs, files in os.walk(old_path):
        ...
        for file in files:
            ...
            if os.path.isfile(file_path) and is_markdown_file(file_path):
                if not is_empty_markdown_file(file_path):
                    ...
                    logging.info("Destination path: " + new_file_path)
                    new_dirname = os.path.split(new_file_path)[0]
                    os.makedirs(new_dirname, exist_ok=True)
                    shutil.copyfile(file_path, new_file_path)
                    # old_to_new_paths[file_path] = new_file_path
